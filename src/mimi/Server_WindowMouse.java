...
public class Server_WindowMouse extends JPanel
{
   	/**
	 * 
	 */
	...
	Server_MousePolice mouse=new Server_MousePolice();
	ServerThread t=new ServerThread();
   	public Server_WindowMouse()
   	{
		setLayout(null);
       	tool=getToolkit();
   	...
   	public void paint(Graphics g)
   	{
   		...
   		if(Server_MousePolice.over==0||Server_MousePolice.needClean==0)
   		{
	   		super.paint(g);//有组件
   		...
   		else if(ServerThread.w==1||Server_MousePolice.over==1||Server_MousePolice.needClean==1)
   		{
   			super.paint(g);
   			g.drawImage(backGround,0,0,520,600,this);//添加背景图片
   			g.drawImage(image,100,100,400,400,this);
   			...
   			for(int i=0;i<19;i++)
   			{
   				for(int j=0;j<19;j++)
   				{
   					ServerThread.board[i][j]=0;
   				}
   			}
			Server_MousePolice.isWin=0;

			Server_MousePolice.over=0;
			Server_MousePolice.needClean=0;
			paint(g);
   		}
   		for(int i=0;i<19;i++)
   		{
